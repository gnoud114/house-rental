...
export const registerService = ({phone, name, password}) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.User.findOrCreate({
            ...
            const token = response[1] && jwt.sign({id: response[0].id, phone: response[0].phone, name: response[0].name}, process.env.SECRET_KEY, {expiresIn: '2d'})
            resolve({
                err: token ? 0 : 2,
                msg: token ? 'Register success' : 'Phonenumber is already in use',
                token: token || null
            })
    } catch (error) {
        reject(error)
